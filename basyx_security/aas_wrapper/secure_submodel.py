...
class SecureSubmodel:
    ...
    def __init__(
        ...
        for element in submodel.submodel_element:
            self._secure_elements[element.id_short] = SecureElement(
                element,
                security_manager
            )
    ...
    def get_element(
        ...
        if not self._security_manager.check_access(
            context,
            self._submodel.id_short,
            AccessRight.READ
        ):
            raise SecurityViolation(
                f"Access denied to read submodel {self._submodel.id_short}"
            )
    ...
    def add_element(
        ...
        if not self._security_manager.check_access(
            ...
            raise SecurityViolation(
                f"Access denied to modify submodel {self._submodel.id_short}"
            )
