...
def create_jwt_token(user_id: str, roles: list[str], expiry_minutes: int = 30) -> str:
    ...
    payload = {"sub": user_id, "roles": roles, "exp": expiry}
...
def create_complex_aas():
    ...
    # Create properties
    temp = model.Property(id_short="Temperature", value_type=model.datatypes.Double, value=25.5)

    humidity = model.Property(id_short="Humidity", value_type=model.datatypes.Double, value=60.0)

    status = model.Property(
        id_short="Status", value_type=model.datatypes.String, value="operational"
    )
    ...
    status_submodel = model.Submodel(
        id_="https://example.com/status", id_short="SystemStatus", submodel_element={status}
    )
