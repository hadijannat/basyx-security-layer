    ...
    with open(file_path, "r", encoding="utf-8") as f:
        for line_num, line in enumerate(f, 1):
            if len(line.rstrip()) > 100:
                print(
                    f"{file_path}:{line_num}: Line too long ({len(line.rstrip())} > 100 characters)"
                )
                return True
    return False
...
def fix_file(file_path: Path) -> None:
    ...
    subprocess.run(["isort", "--profile=black", "--line-length=100", str(file_path)], check=True)


def main():
    """Find and fix all Python files with long lines."""
    # Directories to scan
    scan_dirs = ["sdk", "server", "basyx_security", "tests", "examples"]
