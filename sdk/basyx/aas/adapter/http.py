...
class HTTPApiDecoder:
    ...
    @classmethod
    def json_list(
        cls, data: Union[str, bytes], expect_type: Type[T], stripped: bool, expect_single: bool
    ) -> List[T]:
        cls.check_type_supportance(expect_type)
        decoder: Type[StrictAASFromJsonDecoder] = (
            StrictStrippedAASFromJsonDecoder if stripped else StrictAASFromJsonDecoder
        )
        try:
    ...
    @classmethod
    def base64urljson_list(
        cls, data: str, expect_type: Type[T], stripped: bool, expect_single: bool
    ) -> List[T]:
    ...
    def xml(cls, data: bytes, expect_type: Type[T], stripped: bool) -> T:
        ...
        try:
            ...
            rv = read_aas_xml_element(
                xml_data,
                cls.type_constructables_map[expect_type],
                stripped=stripped,
                failsafe=False,
            )
...
class WSGIApp:
    def __init__(
        ...
        self.url_map = werkzeug.routing.Map(
                ...
                Submount(
                    ...
                    [
                                ...
                                Submount(
                                    ...
                                    [
                                        Rule(
                                            "/$reference",
                                            methods=["GET"],
                                            endpoint=self.get_aas_reference,
                                        ),
                                        Rule(
                                            "/asset-information",
                                            methods=["GET"],
                                        ...
                                        Rule(
                                            "/submodel-refs",
                                            methods=["GET"],
                                            endpoint=self.get_aas_submodel_refs,
                                        ),
                                        Rule(
                                            "/submodel-refs",
                                            methods=["POST"],
                                            endpoint=self.post_aas_submodel_refs,
                                        ),
                                        Rule(
                        ...
                        Submount(
                            ...
                            [
                                Rule(
                                    "/$metadata",
                                    methods=["GET"],
                                    endpoint=self.get_submodel_all_metadata,
                                ),
                                Rule(
                                    "/$reference",
                                    methods=["GET"],
                                    endpoint=self.get_submodel_all_reference,
                                ),
                                ...
                                Rule(
                                    "/<base64url:submodel_id>",
                                    methods=["GET"],
                                    endpoint=self.get_submodel,
                                ),
                                Rule(
                                    "/<base64url:submodel_id>",
                                    methods=["PUT"],
                                    endpoint=self.put_submodel,
                                ),
                                Rule(
                                    "/<base64url:submodel_id>",
                                    methods=["DELETE"],
                                    endpoint=self.delete_submodel,
                                ),
                                Rule(
                                    "/<base64url:submodel_id>",
                                    methods=["PATCH"],
                                ...
                                Submount(
                                    ...
                                    [
                                        Rule(
                                            "/$metadata",
                                            methods=["GET"],
                                            endpoint=self.get_submodels_metadata,
                                        ),
                                        Rule(
                                            "/$metadata",
                                            methods=["PATCH"],
                                        ...
                                        Rule(
                                            "/$reference",
                                            methods=["GET"],
                                            endpoint=self.get_submodels_reference,
                                        ),
                                        Rule(
                        ...
                        Rule(
                            ...
                            methods=["GET"],
                            endpoint=self.get_concept_description_all,
                        ),
                        Rule(
                            "/concept-descriptions",
                            methods=["POST"],
                            endpoint=self.post_concept_description,
                        ),
                        Submount(
                            "/concept-descriptions",
                            [
                                Rule(
                                    ...
                                    methods=["GET"],
                                    endpoint=self.get_concept_description,
                                ),
                                Rule(
                                    "/<base64url:concept_id>",
                                    methods=["PUT"],
                                    endpoint=self.put_concept_description,
                                ),
                                Rule(
                                    "/<base64url:concept_id>",
                                    methods=["DELETE"],
            ...
            converters={"base64url": Base64URLConverter, "id_short_path": IdShortPathConverter},
    ...
    def _get_obj_ts(
        self, identifier: model.Identifier, type_: Type[model.provider._IT]
    ) -> model.provider._IT:
    ...
    def _get_submodel_or_nested_submodel_element(
        self, url_args: Dict
    ...
    def _namespace_submodel_element_op(
        ...
        try:
            ...
            raise NotFound(
                f"Submodel element with id_short {arg} not found in {namespace!r}"
            ) from e

    @classmethod
    def _qualifiable_qualifier_op(
        cls, qualifiable: model.Qualifiable, op: Callable[[str], T], arg: str
    ) -> T:
        try:
            return op(arg)
        except KeyError as e:
    ...
    def _get_shells(self, request: Request) -> Tuple[Iterator[model.AssetAdministrationShell], int]:
        ...
        if asset_ids is not None:
            ...
            specific_asset_ids: List[model.SpecificAssetId] = list(
                map(
                    lambda asset_id: HTTPApiDecoder.base64urljson(
                        asset_id, model.SpecificAssetId, False
                    ),
                    asset_ids,
                )
            )
    ...
    def _get_submodel_submodel_elements_id_short_path(
        self, url_args: Dict
    ) -> model.SubmodelElement:
        submodel = self._get_submodel(url_args)
    ...
    def post_aas(
        ...
        try:
            ...
            raise Conflict(
                f"AssetAdministrationShell with Identifier {aas.id} already exists!"
            ) from e
        aas.commit()
        created_resource_url = map_adapter.build(
            self.get_aas, {"aas_id": aas.id}, force_external=True
        )
    ...
    def get_aas_all_reference(
        ...
        references: list[model.ModelReference] = [
            model.ModelReference.from_referable(aas) for aas in aashells
        ]
    ...
    def put_aas(
        ...
        aas.update_from(
            HTTPApiDecoder.request_body(
                request, model.AssetAdministrationShell, is_stripped_request(request)
            )
        )
        aas.commit()
        return response_t()
    ...
    def aas_submodel_refs_redirect(
        self, request: Request, url_args: Dict, map_adapter: MapAdapter
    ) -> Response:
        aas = self._get_shell(url_args)
    ...
    def post_submodel(
        ...
        submodel.commit()
        created_resource_url = map_adapter.build(
            self.get_submodel, {"submodel_id": submodel.id}, force_external=True
        )
    ...
    def put_submodel(
        ...
        submodel = self._get_submodel(url_args)
        submodel.update_from(
            HTTPApiDecoder.request_body(request, model.Submodel, is_stripped_request(request))
        )
        submodel.commit()
        return response_t()
    ...
    def get_submodel_submodel_elements(
        ...
        return response_t(
            list(submodel_elements), cursor=cursor, stripped=is_stripped_request(request)
        )
    ...
    def post_submodel_submodel_elements_id_short_path(
        ...
        try:
            ...
            if e.constraint_id != 22:
                raise
            raise Conflict(
                f"SubmodelElement with idShort {new_submodel_element.id_short} already exists "
                f"within {parent}!"
            )
        submodel = self._get_submodel(url_args)
        ...
        return response_t(
            new_submodel_element, status=201, headers={"Location": created_resource_url}
        )
    ...
    def get_submodel_submodel_element_attachment(
        self, request: Request, url_args: Dict, **_kwargs
    ) -> Response:
        submodel_element = self._get_submodel_submodel_elements_id_short_path(url_args)
    ...
    def get_submodel_submodel_element_qualifiers(
        ...
        return response_t(
            self._qualifiable_qualifier_op(sm_or_se, sm_or_se.get_qualifier_by_type, qualifier_type)
        )
    ...
    def get_concept_description_all(
        ...
        return response_t(
            list(concept_descriptions), cursor=cursor, stripped=is_stripped_request(request)
        )
    ...
    def post_concept_description(
        ...
        try:
            ...
            raise Conflict(
                f"ConceptDescription with Identifier {concept_description.id} already exists!"
            ) from e
        concept_description.commit()
        created_resource_url = map_adapter.build(
            self.get_concept_description,
            {"concept_id": concept_description.id},
            force_external=True,
        )
        return response_t(
            concept_description, status=201, headers={"Location": created_resource_url}
        )
