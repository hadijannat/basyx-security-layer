...
def _generate_element(
    name: str, text: Optional[str] = None, attributes: Optional[Dict] = None
) -> etree._Element:
    """
...
def abstract_classes_to_xml(tag: str, obj: object) -> etree._Element:
    ...
    if isinstance(obj, model.HasSemantics):
        ...
        if obj.supplemental_semantic_id:
            ...
            for supplemental_semantic_id in obj.supplemental_semantic_id:
                et_supplemental_semantic_ids.append(
                    reference_to_xml(supplemental_semantic_id, NS_AAS + "reference")
                )
    ...
    if isinstance(obj, model.HasDataSpecification):
        if obj.embedded_data_specifications:
            et_embedded_data_specifications = _generate_element(
                NS_AAS + "embeddedDataSpecifications"
            )
...
def _value_to_xml(
    value: model.ValueDataType, value_type: model.DataTypeDefXsd, tag: str = NS_AAS + "value"
) -> etree._Element:
    """
    ...
    return _generate_element(tag, text=model.datatypes.xsd_repr(value))
...
def lang_string_set_to_xml(obj: model.LangStringSet, tag: str) -> etree._Element:
    ...
    :return: Serialized :class:`~lxml.etree._Element` object
    """
    LANG_STRING_SET_TAGS: Dict[Type[model.LangStringSet], str] = {
        k: NS_AAS + v
        for k, v in {
...
def administrative_information_to_xml(
    obj: model.AdministrativeInformation, tag: str = NS_AAS + "administration"
) -> etree._Element:
    """
    ...
    if obj.template_id:
        et_administration.append(
            _generate_element(name=NS_AAS + "templateId", text=obj.template_id)
        )
...
def data_element_to_xml(obj: model.DataElement) -> etree._Element:
    ...
    raise AssertionError(
        f"Type {obj.__class__.__name__} is not yet supported by the XML serialization!"
    )
...
def reference_to_xml(obj: model.Reference, tag: str = NS_AAS + "reference") -> etree._Element:
    ...
    et_reference.append(
        _generate_element(NS_AAS + "type", text=_generic.REFERENCE_TYPES[obj.__class__])
    )
    if obj.referred_semantic_id is not None:
        et_reference.append(
            reference_to_xml(obj.referred_semantic_id, NS_AAS + "referredSemanticId")
        )
...
def qualifier_to_xml(obj: model.Qualifier, tag: str = NS_AAS + "qualifier") -> etree._Element:
    ...
    et_qualifier.append(
        _generate_element(NS_AAS + "valueType", text=model.datatypes.XSD_TYPE_NAMES[obj.value_type])
    )
    if obj.value:
...
def extension_to_xml(obj: model.Extension, tag: str = NS_AAS + "extension") -> etree._Element:
    ...
    if obj.value_type:
        et_extension.append(
            _generate_element(
                NS_AAS + "valueType", text=model.datatypes.XSD_TYPE_NAMES[obj.value_type]
            )
        )
    if obj.value:
...
def value_reference_pair_to_xml(
    obj: model.ValueReferencePair, tag: str = NS_AAS + "valueReferencePair"
) -> etree._Element:
    """
...
def value_list_to_xml(obj: model.ValueList, tag: str = NS_AAS + "valueList") -> etree._Element:
    """
    ...
    for aas_reference_pair in obj:
        et_value_reference_pairs.append(
            value_reference_pair_to_xml(aas_reference_pair, NS_AAS + "valueReferencePair")
        )
...
def specific_asset_id_to_xml(
    obj: model.SpecificAssetId, tag: str = NS_AAS + "specificAssetId"
) -> etree._Element:
    """
    ...
    if obj.external_subject_id:
        et_asset_information.append(
            reference_to_xml(obj.external_subject_id, NS_AAS + "externalSubjectId")
        )

    return et_asset_information


def asset_information_to_xml(
    obj: model.AssetInformation, tag: str = NS_AAS + "assetInformation"
) -> etree._Element:
    """
    ...
    et_asset_information = abstract_classes_to_xml(tag, obj)
    et_asset_information.append(
        _generate_element(name=NS_AAS + "assetKind", text=_generic.ASSET_KIND[obj.asset_kind])
    )
    if obj.global_asset_id:
        et_asset_information.append(
            _generate_element(name=NS_AAS + "globalAssetId", text=obj.global_asset_id)
        )
    if obj.specific_asset_id:
        ...
        for specific_asset_id in obj.specific_asset_id:
            et_specific_asset_id.append(
                specific_asset_id_to_xml(specific_asset_id, NS_AAS + "specificAssetId")
            )
        et_asset_information.append(et_specific_asset_id)
    if obj.asset_type:
        et_asset_information.append(
            _generate_element(name=NS_AAS + "assetType", text=obj.asset_type)
        )
    if obj.default_thumbnail:
        et_asset_information.append(
            resource_to_xml(obj.default_thumbnail, NS_AAS + "defaultThumbnail")
        )

    return et_asset_information


def concept_description_to_xml(
    obj: model.ConceptDescription, tag: str = NS_AAS + "conceptDescription"
) -> etree._Element:
    """
...
def embedded_data_specification_to_xml(
    obj: model.EmbeddedDataSpecification, tag: str = NS_AAS + "embeddedDataSpecification"
) -> etree._Element:
    """
    ...
    et_embedded_data_specification.append(
        reference_to_xml(obj.data_specification, tag=NS_AAS + "dataSpecification")
    )
    et_embedded_data_specification.append(
        data_specification_content_to_xml(obj.data_specification_content)
    )
    return et_embedded_data_specification


def data_specification_content_to_xml(
    obj: model.DataSpecificationContent, tag: str = NS_AAS + "dataSpecificationContent"
) -> etree._Element:
    """
...
def data_specification_iec61360_to_xml(
    obj: model.DataSpecificationIEC61360, tag: str = NS_AAS + "dataSpecificationIec61360"
) -> etree._Element:
    """
    ...
    et_data_specification_iec61360.append(
        lang_string_set_to_xml(obj.preferred_name, NS_AAS + "preferredName")
    )
    if obj.short_name is not None:
        et_data_specification_iec61360.append(
            lang_string_set_to_xml(obj.short_name, NS_AAS + "shortName")
        )
    ...
    if obj.source_of_definition is not None:
        et_data_specification_iec61360.append(
            _generate_element(NS_AAS + "sourceOfDefinition", text=obj.source_of_definition)
        )
    ...
    if obj.data_type is not None:
        et_data_specification_iec61360.append(
            _generate_element(NS_AAS + "dataType", text=_generic.IEC61360_DATA_TYPES[obj.data_type])
        )
    if obj.definition is not None:
        et_data_specification_iec61360.append(
            lang_string_set_to_xml(obj.definition, NS_AAS + "definition")
        )

    if obj.value_format is not None:
        et_data_specification_iec61360.append(
            _generate_element(NS_AAS + "valueFormat", text=obj.value_format)
        )
    ...
    if obj.level_types:
        ...
        for k, v in _generic.IEC61360_LEVEL_TYPES.items():
            et_level_types.append(
                _generate_element(NS_AAS + v, text=boolean_to_xml(k in obj.level_types))
            )
        et_data_specification_iec61360.append(et_level_types)
    return et_data_specification_iec61360


def asset_administration_shell_to_xml(
    obj: model.AssetAdministrationShell, tag: str = NS_AAS + "assetAdministrationShell"
) -> etree._Element:
    """
...
def submodel_to_xml(obj: model.Submodel, tag: str = NS_AAS + "submodel") -> etree._Element:
    """
...
def property_to_xml(obj: model.Property, tag: str = NS_AAS + "property") -> etree._Element:
    """
...
def multi_language_property_to_xml(
    obj: model.MultiLanguageProperty, tag: str = NS_AAS + "multiLanguageProperty"
) -> etree._Element:
    """
...
def range_to_xml(obj: model.Range, tag: str = NS_AAS + "range") -> etree._Element:
    """
    ...
    et_range.append(
        _generate_element(
            name=NS_AAS + "valueType", text=model.datatypes.XSD_TYPE_NAMES[obj.value_type]
        )
    )
...
def blob_to_xml(obj: model.Blob, tag: str = NS_AAS + "blob") -> etree._Element:
    """
...
def file_to_xml(obj: model.File, tag: str = NS_AAS + "file") -> etree._Element:
    """
...
def resource_to_xml(obj: model.Resource, tag: str = NS_AAS + "resource") -> etree._Element:
    """
...
def reference_element_to_xml(
    obj: model.ReferenceElement, tag: str = NS_AAS + "referenceElement"
) -> etree._Element:
    """
...
def submodel_element_collection_to_xml(
    obj: model.SubmodelElementCollection, tag: str = NS_AAS + "submodelElementCollection"
) -> etree._Element:
    """
...
def submodel_element_list_to_xml(
    obj: model.SubmodelElementList, tag: str = NS_AAS + "submodelElementList"
) -> etree._Element:
    """
    ...
    et_submodel_element_list.append(
        _generate_element(NS_AAS + "orderRelevant", boolean_to_xml(obj.order_relevant))
    )
    if obj.semantic_id_list_element is not None:
        et_submodel_element_list.append(
            reference_to_xml(obj.semantic_id_list_element, NS_AAS + "semanticIdListElement")
        )
    et_submodel_element_list.append(
        _generate_element(
            NS_AAS + "typeValueListElement",
            _generic.KEY_TYPES[model.KEY_TYPES_CLASSES[obj.type_value_list_element]],
        )
    )
    if obj.value_type_list_element is not None:
        et_submodel_element_list.append(
            _generate_element(
                NS_AAS + "valueTypeListElement",
...
def relationship_element_to_xml(
    obj: model.RelationshipElement, tag: str = NS_AAS + "relationshipElement"
) -> etree._Element:
    """
...
def annotated_relationship_element_to_xml(
    obj: model.AnnotatedRelationshipElement, tag: str = NS_AAS + "annotatedRelationshipElement"
) -> etree._Element:
    """
...
def operation_variable_to_xml(
    obj: model.SubmodelElement, tag: str = NS_AAS + "operationVariable"
) -> etree._Element:
    """
...
def operation_to_xml(obj: model.Operation, tag: str = NS_AAS + "operation") -> etree._Element:
    """
    ...
    for tag, nss in (
        (NS_AAS + "inputVariables", obj.input_variable),
...
def capability_to_xml(obj: model.Capability, tag: str = NS_AAS + "capability") -> etree._Element:
    """
...
def entity_to_xml(obj: model.Entity, tag: str = NS_AAS + "entity") -> etree._Element:
    """
    ...
    et_entity.append(
        _generate_element(NS_AAS + "entityType", text=_generic.ENTITY_TYPES[obj.entity_type])
    )
    if obj.global_asset_id:
...
def basic_event_element_to_xml(
    obj: model.BasicEventElement, tag: str = NS_AAS + "basicEventElement"
) -> etree._Element:
    """
    ...
    et_basic_event_element.append(
        _generate_element(NS_AAS + "direction", text=_generic.DIRECTION[obj.direction])
    )
    et_basic_event_element.append(
        _generate_element(NS_AAS + "state", text=_generic.STATE_OF_EVENT[obj.state])
    )
    if obj.message_topic is not None:
        et_basic_event_element.append(
            _generate_element(NS_AAS + "messageTopic", text=obj.message_topic)
        )
    if obj.message_broker is not None:
        et_basic_event_element.append(
            reference_to_xml(obj.message_broker, NS_AAS + "messageBroker")
        )
    if obj.last_update is not None:
        et_basic_event_element.append(
            _generate_element(NS_AAS + "lastUpdate", text=model.datatypes.xsd_repr(obj.last_update))
        )
    if obj.min_interval is not None:
        et_basic_event_element.append(
            _generate_element(
                NS_AAS + "minInterval", text=model.datatypes.xsd_repr(obj.min_interval)
            )
        )
    if obj.max_interval is not None:
        et_basic_event_element.append(
            _generate_element(
                NS_AAS + "maxInterval", text=model.datatypes.xsd_repr(obj.max_interval)
            )
        )
...
def _write_element(file: _generic.PathOrBinaryIO, element: etree._Element, **kwargs) -> None:
    etree.ElementTree(element).write(
        file, encoding="UTF-8", xml_declaration=True, method="xml", **kwargs
    )
...
def write_aas_xml_file(
    file: _generic.PathOrBinaryIO, data: model.AbstractObjectStore, **kwargs
) -> None:
    """
