    ...
    def raise_failed(self) -> None:
        ...
        if len(failed) > 0:
            raise AssertionError(
                "{} of {} checks failed".format(len(failed), len(self.checks)),
...
class AASDataChecker(DataChecker):
    ...
    def _check_submodel_element(
        self, object_: model.SubmodelElement, expected_object: model.SubmodelElement
    ):
    ...
    def _check_has_extension_equal(
        self, object_: model.HasExtension, expected_object: model.HasExtension
    ):
        """
        ...
        if not self.check_extensions:
            return
        self.check_contained_element_length(
            object_, "extension", model.Extension, len(expected_object.extension)
        )
        ...
        found_extensions = self._find_extra_namespace_set_elements_by_name(
            object_.extension, expected_object.extension
        )
        self.check(
            found_extensions == set(),
            f"{object_!r} must not have extra extensions",
            value=found_extensions,
        )
    ...
    def _check_identifiable_equal(
        self, object_: model.Identifiable, expected_object: model.Identifiable
    ):
        """
        ...
        if object_.administration is not None and expected_object.administration is not None:
            self._check_has_data_specification_equal(
                object_.administration, expected_object.administration
            )
        self.check_attribute_equal(object_, "id", expected_object.id)

    def _check_has_semantics_equal(
        self, object_: model.HasSemantics, expected_object: model.HasSemantics
    ):
        """
        ...
        for suppl_semantic_id in expected_object.supplemental_semantic_id:
            given_semantic_id = self._find_reference(
                suppl_semantic_id, object_.supplemental_semantic_id
            )
            self.check(
                given_semantic_id is not None,
                f"{object_!r} must have supplementalSemanticId",
                value=suppl_semantic_id,
            )

        found_elements = self._find_extra_object(
            object_.supplemental_semantic_id,
            expected_object.supplemental_semantic_id,
            model.Reference,
        )
        self.check(
            found_elements == set(),
            "{} must not have extra supplementalSemanticId".format(repr(object_)),
    ...
    def _check_qualifiable_equal(
        self, object_: model.Qualifiable, expected_object: model.Qualifiable
    ):
        """
        ...
        """
        self.check_contained_element_length(
            object_, "qualifier", model.Qualifier, len(expected_object.qualifier)
        )
        for expected_element in expected_object.qualifier:
            element = self._find_element_by_attribute(
                expected_element, list(object_.qualifier), "type"
            )
        ...
        found_elements = self._find_extra_elements_by_attribute(
            list(object_.qualifier), list(expected_object.qualifier), "type"
        )
        self.check(
            found_elements == set(),
            "Qualifiable Element {} must not have extra elements".format(repr(object_)),
            value=found_elements,
        )

    def _check_has_data_specification_equal(
        self, object_: model.HasDataSpecification, expected_object: model.HasDataSpecification
    ):
        """
        ...
        for expected_dspec in expected_object.embedded_data_specifications:
            given_dspec = self._find_element_by_attribute(
                expected_dspec, object_.embedded_data_specifications, "data_specification"
            )
            if self.check(
                given_dspec is not None,
                "EmbeddedDataSpecification {} must exist in {}".format(
                    repr(expected_dspec.data_specification), repr(object_)
                ),
            ):
                self.check_data_specification_content_equal(
                    given_dspec.data_specification_content,  # type: ignore
                    expected_dspec.data_specification_content,
                )

        found_elements = self._find_extra_elements_by_attribute(
            object_.embedded_data_specifications,
            expected_object.embedded_data_specifications,
            "data_specification",
        )
        self.check(
            found_elements == set(),
            "{} must not have extra data specifications".format(repr(object_)),
            value=found_elements,
        )

    def _check_abstract_attributes_submodel_element_equal(
        self, object_: model.SubmodelElement, expected_value: model.SubmodelElement
    ):
        """
    ...
    def _check_submodel_elements_equal_unordered(
        self, object_: _LIST_OR_COLLECTION, expected_value: _LIST_OR_COLLECTION
    ):
        """
        ...
        found_elements = self._find_extra_namespace_set_elements_by_id_short(
            object_.value, expected_value.value
        )
        self.check(
            found_elements == set(),
            "{} must not have extra elements".format(repr(object_)),
            value=found_elements,
        )
    ...
    def check_multi_language_property_equal(
        self, object_: model.MultiLanguageProperty, expected_value: model.MultiLanguageProperty
    ):
        """
    ...
    def check_reference_element_equal(
        self, object_: model.ReferenceElement, expected_value: model.ReferenceElement
    ):
        """
    ...
    def check_submodel_element_collection_equal(
        self,
        object_: model.SubmodelElementCollection,
        ...
        self._check_abstract_attributes_submodel_element_equal(object_, expected_value)
        self.check_contained_element_length(
            object_, "value", model.SubmodelElement, len(expected_value.value)
        )
        self._check_submodel_elements_equal_unordered(object_, expected_value)

    def check_submodel_element_list_equal(
        self, object_: model.SubmodelElementList, expected_value: model.SubmodelElementList
    ):
        """
        ...
        self.check_attribute_equal(
            object_, "semantic_id_list_element", expected_value.semantic_id_list_element
        )
        self.check_attribute_equal(
            object_, "value_type_list_element", expected_value.value_type_list_element
        )
        self.check_attribute_equal(
            object_, "type_value_list_element", expected_value.type_value_list_element
        )
        self.check_contained_element_length(
            object_, "value", object_.type_value_list_element, len(expected_value.value)
        )
        if not object_.order_relevant or not expected_value.order_relevant:
            ...
            raise NotImplementedError(
                "A SubmodelElementList with order_relevant=False cannot be compared!"
            )
    ...
    def check_relationship_element_equal(
        self, object_: model.RelationshipElement, expected_value: model.RelationshipElement
    ):
        """
    ...
    def check_annotated_relationship_element_equal(
        self,
        object_: model.AnnotatedRelationshipElement,
        ...
        self.check_contained_element_length(
            object_, "annotation", model.DataElement, len(expected_value.annotation)
        )
        ...
        found_elements = self._find_extra_namespace_set_elements_by_id_short(
            object_.annotation, expected_value.annotation
        )
        self.check(
            found_elements == set(),
            "Annotated Reference {} must not have extra " "references".format(repr(object_)),
            value=found_elements,
        )
    ...
    def _check_reference_equal(self, object_: model.Reference, expected_value: model.Reference):
        ...
        """
        self.check(
            object_ == expected_value,
            "{} must be == {}".format(repr(object_), repr(expected_value)),
        )

    def _find_reference(
        self, object_: model.Reference, search_list: Iterable
    ) -> Union[model.Reference, None]:
        """
    ...
    def _find_specific_asset_id(
        self, object_: model.SpecificAssetId, search_list: Iterable
    ...
    def _find_element_by_attribute(
        self, object_: object, search_list: Iterable, *attribute: str
    ) -> object:
        """
    ...
    def _find_extra_object(
        self, object_list: Iterable, search_list: Iterable, type_
    ) -> Union[Set, None]:
        """
    ...
    def _find_extra_elements_by_attribute(
        self, object_list: Union[Set, List], search_list: Union[Set, List], *attribute: str
    ) -> Set:
        """
    ...
    def _find_extra_namespace_set_elements_by_attribute(
        self, object_list: model.NamespaceSet, search_list: model.NamespaceSet, attr_name: str
    ) -> Set:
        """
    ...
    def _find_extra_namespace_set_elements_by_id_short(
        self, object_list: model.NamespaceSet, search_list: model.NamespaceSet
    ) -> Set:
        """
        ...
        :return: Set of elements that are in object_list but not in search_list
        """
        return self._find_extra_namespace_set_elements_by_attribute(
            object_list, search_list, "id_short"
        )

    def _find_extra_namespace_set_elements_by_name(
        self, object_list: model.NamespaceSet, search_list: model.NamespaceSet
    ) -> Set:
        """
    ...
    def check_operation_equal(self, object_: model.Operation, expected_value: model.Operation):
        ...
        for input_nss, expected_nss, attr_name in (
            ...
            self.check_contained_element_length(
                object_, attr_name, model.SubmodelElement, len(expected_nss)
            )
    ...
    def check_entity_equal(self, object_: model.Entity, expected_value: model.Entity):
        ...
        self._check_specific_asset_ids_equal(
            object_.specific_asset_id, expected_value.specific_asset_id, object_
        )
        self.check_contained_element_length(
            object_, "statement", model.SubmodelElement, len(expected_value.statement)
        )
        ...
        found_elements = self._find_extra_namespace_set_elements_by_id_short(
            object_.statement, expected_value.statement
        )
        self.check(
            found_elements == set(),
            f"Entity {repr(object_)} must not have extra statements",
            value=found_elements,
        )

    def _check_specific_asset_ids_equal(
        self,
        object_: Iterable[model.SpecificAssetId],
        ...
        self.check(
            found_elements == set(),
            f"{repr(object_parent)} must not have extra specificAssetIds",
            value=found_elements,
        )

    def _check_event_element_equal(
        self, object_: model.EventElement, expected_value: model.EventElement
    ):
        """
    ...
    def check_basic_event_element_equal(
        self, object_: model.BasicEventElement, expected_value: model.BasicEventElement
    ):
        """
    ...
    def check_submodel_equal(self, object_: model.Submodel, expected_value: model.Submodel):
        ...
        self._check_has_data_specification_equal(object_, expected_value)
        self.check_contained_element_length(
            object_, "submodel_element", model.SubmodelElement, len(expected_value.submodel_element)
        )
        ...
        found_elements = self._find_extra_namespace_set_elements_by_id_short(
            object_.submodel_element, expected_value.submodel_element
        )
        self.check(
            found_elements == set(),
            "Submodel {} must not have extra submodel elements".format(repr(object_)),
            value=found_elements,
        )
    ...
    def check_specific_asset_id(
        self, object_: model.SpecificAssetId, expected_value: model.SpecificAssetId
    ):
        """
        ...
        self.check_attribute_equal(object_, "value", expected_value.value)
        self.check_attribute_equal(
            object_, "external_subject_id", expected_value.external_subject_id
        )
        self.check_attribute_equal(object_, "semantic_id", expected_value.semantic_id)

    def check_asset_information_equal(
        self, object_: model.AssetInformation, expected_value: model.AssetInformation
    ):
        """
        ...
        if object_.default_thumbnail and expected_value.default_thumbnail:
            ...
            if object_.default_thumbnail:
                self.check(
                    expected_value.default_thumbnail is not None,
                    "defaultThumbnail object {} must exist".format(repr(object_.default_thumbnail)),
                    value=expected_value.default_thumbnail,
                )
            else:
                self.check(
                    expected_value.default_thumbnail is None,
                    "{} must not have a " "defaultThumbnail object".format(repr(object_)),
                    value=expected_value.default_thumbnail,
                )

    def check_asset_administration_shell_equal(
        self,
        object_: model.AssetAdministrationShell,
        ...
        self._check_identifiable_equal(object_, expected_value)
        self._check_has_data_specification_equal(object_, expected_value)
        self.check_asset_information_equal(
            object_.asset_information, expected_value.asset_information
        )

        self.check_attribute_equal(object_, "derived_from", expected_value.derived_from)
        self.check_contained_element_length(
            object_, "submodel", model.ModelReference, len(expected_value.submodel)
        )
        for expected_ref in expected_value.submodel:
            ref = self._find_reference(expected_ref, object_.submodel)
            if self.check(
                ref is not None, "Submodel Reference {} must exist".format(repr(expected_ref))
            ):
                self._check_reference_equal(ref, expected_ref)  # type: ignore

        found_elements = self._find_extra_object(
            object_.submodel, expected_value.submodel, model.ModelReference
        )
        self.check(
            found_elements == set(),
            "Asset Administration Shell {} must not have extra submodel "
            "references".format(repr(object_)),
            value=found_elements,
        )

    def check_concept_description_equal(
        self, object_: model.ConceptDescription, expected_value: model.ConceptDescription
    ):
        """
        ...
        self._check_has_data_specification_equal(object_, expected_value)
        self.check_contained_element_length(
            object_, "is_case_of", model.Reference, len(expected_value.is_case_of)
        )
        for expected_ref in expected_value.is_case_of:
            ref = self._find_reference(expected_ref, object_.is_case_of)
            if self.check(
                ref is not None,
                "Concept Description Reference {} must exist".format(repr(expected_ref)),
            ):
                self._check_reference_equal(ref, expected_ref)  # type: ignore

        found_elements = self._find_extra_object(
            object_.is_case_of, expected_value.is_case_of, model.ModelReference
        )
        self.check(
            found_elements == set(),
            "Concept Description Reference {} must not have extra "
    ...
    def check_data_specification_content_equal(
        self,
        object_: model.DataSpecificationContent,
        ...
        """
        self.check(
            type(object_) is type(expected_value),
            "type({}) must be == type({})".format(repr(object_), repr(expected_value)),
        )
    ...
    def _check_data_specification_iec61360_equal(
        self,
        object_: model.base.DataSpecificationIEC61360,
        ...
        self.check_attribute_equal(
            object_, "source_of_definition", expected_value.source_of_definition
        )
        ...
        if expected_value.value_list is not None:
            if self.check(
                object_.value_list is not None,
                "ValueList must contain {} ValueReferencePairs".format(
        ...
        if object_.value_list is not None:
            if self.check(
                expected_value.value_list is not None,
                "ValueList must contain 0 ValueReferencePairs",
    ...
    def _check_value_list_equal(self, object_: model.ValueList, expected_value: model.ValueList):
        ...
        for expected_pair in expected_value:
            ...
            self.check(
                pair is not None,
                "ValueReferencePair[value={}, value_id={}] "
                "must exist".format(expected_pair.value, expected_pair.value_id),
            )

        found_elements = self._find_extra_elements_by_attribute(
            object_, expected_value, "value", "value_id"
        )
        self.check(
            found_elements == set(),
            "ValueList must not have extra ValueReferencePairs",
            value=found_elements,
        )

    def check_object_store(
        self, obj_store_1: model.DictObjectStore, obj_store_2: model.DictObjectStore
    ):
        """
        ...
        for shell_2 in shell_list_2:
            ...
            if self.check(
                shell_1 is not None,
                "Asset administration shell {} must exist in given asset administration"
        ...
        self.check(
            found_elements == set(),
            "Given asset administration shell list must not have extra asset "
        ...
        for submodel_2 in submodel_list_2:
            ...
            if self.check(
                submodel_1 is not None,
                "Submodel {} must exist in given submodel list".format(submodel_2),
            ):
                self.check_submodel_equal(submodel_1, submodel_2)  # type: ignore

        found_elements = self._find_extra_elements_by_attribute(
            submodel_list_1, submodel_list_2, "id"
        )
        self.check(
            found_elements == set(),
            "Given submodel list must not have extra submodels",
            value=found_elements,
        )
        ...
        for cd_2 in concept_description_list_2:
            ...
            if self.check(
                cd_1 is not None,
                "Concept description {} must exist in given concept description "
                "list".format(cd_2),
            ):
                self.check_concept_description_equal(cd_1, cd_2)  # type: ignore

        found_elements = self._find_extra_elements_by_attribute(
            concept_description_list_1, concept_description_list_2, "id"
        )
        self.check(
            found_elements == set(),
            "Given concept description list must not have extra concept " "descriptions",
            value=found_elements,
        )

    def check_attribute_equal(
        self, object_: object, attribute_name: str, expected_value: object, **kwargs
    ) -> bool:
        """
        ...
        if (
            attribute_name in ("value_type", "value_type_list_element", "type_value_list_element")
            ...
            return self.check(
                getattr(object_, attribute_name) == expected_value,
                "Attribute {} of {} must be == {}".format(
                    attribute_name, repr(object_), expected_value
                ),
                **kwargs,
            )
    ...
    def check_contained_element_length(
        self, object_: object, attribute_name: str, class_name: Type, length: int, **kwargs
    ) -> bool:
        """
        ...
        return self.check(
            count == length,
            "Attribute {} of {} must contain {} {}s".format(
                attribute_name, repr(object_), length, class_name.__name__
            ),
            **kwargs,
        )
    ...
    def check_attribute_is_none(self, object_: object, attribute_name: str, **kwargs) -> bool:
        ...
        kwargs["value"] = getattr(object_, attribute_name)
        return self.check(
            getattr(object_, attribute_name) is None,
