...
class SecureElement:
    ...
    def set_value(self, context: SecurityContext, value: Any):
        ...
        if not hasattr(self._element, "value"):
            raise ValueError(
                f"Element {self._element.id_short} does not support value modification"
            )
...
class SecureSubmodel:
    ...
    def add_element(self, context: SecurityContext, element: model.SubmodelElement):
        ...
        logging.info(
            f"Element {element.id_short} added to {self._submodel.id} by {context.user_id}"
        )
            ...
            for submodel_ref in self._aas.submodel:
                try:
                    ...
                    if submodel.id_short == submodel_id:
                        self._secure_submodels[submodel_id] = SecureSubmodel(
                            submodel, self._security_manager
                        )
