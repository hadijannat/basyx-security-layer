...
class SecurityContext:
    ...
    def __init__(self, user_id: str, roles: Set[str], security_level: SecurityLevel,
                 token: Optional[str] = None, metadata: Optional[Dict] = None):
...
def create_security_context(user_id: str, roles: Set[str], security_level: SecurityLevel,
                          metadata: Optional[Dict] = None) -> SecurityContext:
...
class SecurityManager:
    ...
    def set_role_permissions(self, role: str, resource_id: str, access: AccessRight):
        ...
        logging.info(f"Role permissions set - Role: {role}, Resource: {resource_id}, Access: {access}")
    
    def check_access(self, context: SecurityContext, resource_id: str,
                    required_access: AccessRight) -> bool:
        ...
        if context.security_level.value < required_level.value:
            logging.warning(f"Security level violation - User: {context.user_id}, "
        ...
        for role in context.roles:
            ...
            if required_access == AccessRight.WRITE and access_right.value >= AccessRight.WRITE.value:
                return True
            
            if required_access == AccessRight.EXECUTE and access_right.value >= AccessRight.EXECUTE.value:
                return True
        
        logging.warning(f"Access denied - User: {context.user_id}, Resource: {resource_id}, "
                       f"Required: {required_access}")
        return False
    
    def create_token(self, context: SecurityContext, expiry: datetime.timedelta = datetime.timedelta(hours=1)) -> str:
