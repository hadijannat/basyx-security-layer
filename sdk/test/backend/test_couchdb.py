...
class CouchDBBackendOfflineMethodsTest(unittest.TestCase):
    def test_parse_source(self):
        ...
        with self.assertRaises(couchdb.CouchDBSourceError) as cm:
            couchdb.CouchDBBackend._parse_source(
...
class CouchDBBackendTest(unittest.TestCase):
    ...
    def test_object_store_add(self):
        ...
        self.assertEqual(
            test_object.source, source_core + "https%3A%2F%2Facplt.org%2FTest_Submodel"
        )
    ...
    def test_retrieval(self):
        ...
        test_object_retrieved.source = (
            "couchdb://example.com/example/https%3A%2F%2Facplt.org%2FTest_Submodel"
        )
    ...
    def test_iterating(self) -> None:
        ...
        retrieved_data_store: model.provider.DictObjectStore[model.Identifiable] = (
            model.provider.DictObjectStore()
        )
    ...
    def test_key_errors(self) -> None:
        ...
        with self.assertRaises(KeyError) as cm:
            self.object_store.add(example_submodel)
        self.assertEqual(
            "'Identifiable with id https://acplt.org/Test_Submodel already exists in "
            "CouchDB database'",
            str(cm.exception),
        )
        ...
        self.assertEqual(
            "'No Identifiable with id https://acplt.org/Test_Submodel found in CouchDB database'",
            str(cm.exception),
        )
        ...
        self.assertEqual(
            "'No AAS object with id https://acplt.org/Test_Submodel exists in " "CouchDB database'",
            str(cm.exception),
        )
    ...
    def test_conflict_errors(self):
        ...
        with self.assertRaises(couchdb.CouchDBConflictError) as cm:
            retrieved_submodel.commit()
        self.assertEqual(
            "Could not commit changes to id https://acplt.org/Test_Submodel due to a "
        ...
        self.assertEqual(
            "Object with id https://acplt.org/Test_Submodel has been modified in the "
