...
class DataCheckerTest(unittest.TestCase):
    ...
    def test_kwargs(self):
        ...
        self.assertEqual(
            "('Check failed: Assertion test 1', {'value': 'kwargs1'})", str(cm.exception)
        )
        self.assertEqual(
            "('Check failed: Assertion test 2', {'value': 'kwargs2'})", str(cm_2.exception)
        )
...
class AASDataCheckerTest(unittest.TestCase):
    ...
    def test_qualifiable_checker(self):
        ...
        property = model.Property(id_short="Prop1", value_type=model.datatypes.String, value="test")
        ...
        self.assertEqual(
            "FAIL: Attribute qualifier of Property[Prop1] must contain 1 Qualifiers (count=0)",
    ...
    def test_submodel_element_list_checker(self):
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute max of Range[test_list[0]] must be == 1337 (value=142857)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Attribute max of Range[test_list[1]] must be == 142857 (value=1337)",
            repr(next(checker_iterator)),
        )
        ...
        with self.assertRaises(NotImplementedError) as cm:
            checker.check_submodel_element_list_equal(list_, list_expected)
        self.assertEqual(
            "A SubmodelElementList with order_relevant=False cannot be compared!", str(cm.exception)
        )
        self.assertEqual(1, sum(1 for _ in checker.failed_checks))
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute order_relevant of SubmodelElementList[test_list] must be == True "
            "(value=False)",
            repr(next(checker_iterator)),
        )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute value_type_list_element of SubmodelElementList[test_list] must be == str "
            "(value='Int')",
            repr(next(checker_iterator)),
        )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute type_value_list_element of SubmodelElementList[test_list] must be == "
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute semantic_id_list_element of SubmodelElementList[test_list] must be == "
    ...
    def test_submodel_element_collection_checker(self):
        property = model.Property(id_short="Prop1", value_type=model.datatypes.String, value="test")
        range_ = model.Range(id_short="Range1", value_type=model.datatypes.Int, min=100, max=200)
        collection = model.SubmodelElementCollection(id_short="Collection", value=(range_,))
        property_expected = model.Property(
            id_short="Prop1", value_type=model.datatypes.String, value="test"
        )
        range_expected = model.Range(
            id_short="Range1", value_type=model.datatypes.Int, min=100, max=200
        )
        collection_expected = model.SubmodelElementCollection(
            id_short="Collection", value=(property_expected, range_expected)
        )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute value of SubmodelElementCollection[Collection] must contain 2 "
            "SubmodelElements (count=1)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Submodel Element Property[Collection / Prop1] must exist ()",
            repr(next(checker_iterator)),
        )
    ...
    def test_not_implemented(self):
        ...
        with self.assertRaises(AttributeError) as cm:
            checker.check_submodel_element_collection_equal(
                submodel_collection, submodel_collection
            )
        self.assertEqual("Submodel Element class not implemented", str(cm.exception))

    def test_annotated_relationship_element(self):
        rel1 = model.AnnotatedRelationshipElement(
            id_short="test",
            first=model.ModelReference(
                (
                    model.Key(
                        type_=model.KeyTypes.SUBMODEL, value="http://acplt.org/Test_Submodel"
                    ),
        ...
        rel2 = model.AnnotatedRelationshipElement(
            id_short="test",
            first=model.ModelReference(
                (
                    model.Key(
            ...
            annotation={
                model.Property(
                    id_short="ExampleAnnotatedProperty",
                    value_type=model.datatypes.String,
                    value="exampleValue",
                    parent=None,
                )
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute annotation of AnnotatedRelationshipElement[test] must contain 1 DataElements "
            "(count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Annotation Property[test / ExampleAnnotatedProperty] must exist ()",
            repr(next(checker_iterator)),
        )
    ...
    def test_submodel_checker(self):
        ...
        submodel_expected = model.Submodel(id_="test", submodel_element=(property_expected,))
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute submodel_element of Submodel[test] must contain 1 "
            "SubmodelElements (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Submodel Element Property[test / Prop1] must exist ()",
            repr(next(checker_iterator)),
        )
    ...
    def test_asset_administration_shell_checker(self):
        ...
        shell_expected = model.AssetAdministrationShell(
            ...
            submodel={
                model.ModelReference(
                    (model.Key(type_=model.KeyTypes.SUBMODEL, value="test"),), model.Submodel
                )
            },
        )
        checker = AASDataChecker(raise_immediately=False)
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute submodel of AssetAdministrationShell[test] must contain 1 "
            "ModelReferences (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Submodel Reference ModelReference<Submodel>(key=(Key(type=SUBMODEL,"
    ...
    def test_concept_description_checker(self):
        ...
        cd_expected = model.ConceptDescription(
            id_="test",
            is_case_of={
        ...
        checker_iterator = checker.failed_checks
        self.assertEqual(
            "FAIL: Attribute is_case_of of ConceptDescription[test] must contain "
            "1 References (count=0)",
            repr(next(checker_iterator)),
        )
        self.assertEqual(
            "FAIL: Concept Description Reference ExternalReference(key=(Key("
