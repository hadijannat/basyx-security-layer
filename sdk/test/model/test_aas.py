...
class AssetInformationTest(unittest.TestCase):
    def test_aasd_131_init(self) -> None:
        ...
        model.AssetInformation(
            model.AssetKind.INSTANCE, global_asset_id="https://acplt.org/TestAsset"
        )
        model.AssetInformation(
            model.AssetKind.INSTANCE, specific_asset_id=(model.SpecificAssetId("test", "test"),)
        )
        model.AssetInformation(
    ...
    def test_aasd_131_specific_asset_id_del(self) -> None:
        ...
        asset_information = model.AssetInformation(
            model.AssetKind.INSTANCE,
            specific_asset_id=(model.SpecificAssetId("test1", "test1"), specific_asset_id),
        )
        with self.assertRaises(model.AASConstraintViolation) as cm:
