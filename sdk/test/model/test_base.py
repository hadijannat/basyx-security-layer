...
class KeyTest(unittest.TestCase):
    ...
    def test_from_referable(self):
        ...
        self.assertEqual(
            model.Key(model.KeyTypes.MULTI_LANGUAGE_PROPERTY, "0"), model.Key.from_referable(mlp1)
        )
        self.assertEqual(
            model.Key(model.KeyTypes.MULTI_LANGUAGE_PROPERTY, "1"), model.Key.from_referable(mlp2)
        )
        del se_list.value[0]
        self.assertEqual(
            model.Key(model.KeyTypes.MULTI_LANGUAGE_PROPERTY, "0"), model.Key.from_referable(mlp2)
        )
        with self.assertRaises(ValueError) as cm:
            model.Key.from_referable(mlp1)
        self.assertEqual(
            "Can't create Key for MultiLanguageProperty without an id_short!", str(cm.exception)
        )
        mlp1.id_short = "mlp1"
        self.assertEqual(
...
class ReferableTest(unittest.TestCase):
    def test_id_short_constraint_aasd_002(self):
        ...
        self.assertEqual(
            "The id_short must start with a letter (Constraint AASd-002)", str(cm.exception)
        )
        with self.assertRaises(model.AASConstraintViolation) as cm:
    ...
    def test_update(self):
        ...
        example_grandchild = example_referable.get_referable("exampleChild").get_referable(
            "exampleGrandchild"
        )
        ...
        MockBackend.update_object.assert_has_calls(
            [
                ...
                mock.call(
                    updated_object=example_grandchild,
        ...
        example_referable.update(recursive=False)
        MockBackend.update_object.assert_called_once_with(
            updated_object=example_referable, store_object=example_referable, relative_path=[]
        )
        MockBackend.update_object.reset_mock()
    ...
    def test_commit(self):
        ...
        MockBackend.commit_object.assert_has_calls(
            [
                mock.call(
                    committed_object=example_referable,
                    store_object=example_grandparent,
                    relative_path=["exampleParent", "exampleReferable"],
                ),
                mock.call(
                    committed_object=example_grandchild,
                    store_object=example_grandchild,
                    relative_path=[],
                ),
            ]
    ...
    def test_update_from(self):
        ...
        self.assertIs(
            example_relel,
            example_submodel.submodel_element.get("id_short", "ExampleRelationshipElement"),
        )
...
class ModelNamespaceTest(unittest.TestCase):
    ...
    def setUp(self):
        self.propSemanticID = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://acplt.org/Test1"),)
        )
        self.propSemanticID2 = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://acplt.org/Test2"),)
        )
        self.propSemanticID3 = model.ExternalReference(
            (model.Key(type_=model.KeyTypes.GLOBAL_REFERENCE, value="http://acplt.org/Test3"),)
        )
    ...
    def test_Namespaceset_update_from(self) -> None:
        ...
        namespace2 = self._namespace_class()
        namespace2.set2.add(
            model.Property("Prop1", model.datatypes.Int, 0, semantic_id=self.propSemanticID)
        )
        namespace2.set2.add(
            model.Property("Prop3", model.datatypes.Int, 2, semantic_id=self.propSemanticID2)
        )
...
class ConstrainedListTest(unittest.TestCase):
    ...
    def test_hooks(self) -> None:
        ...
        c_list: model.ConstrainedList[int] = model.ConstrainedList(
            [1, 2, 3], item_add_hook=add_hook, item_set_hook=set_hook, item_del_hook=del_hook
        )
...
class LangStringSetTest(unittest.TestCase):
    def test_language_tag_constraints(self) -> None:
        ...
        self.assertEqual(
            "The language code of the language tag must consist of exactly two lower-case letters! "
            "Given language tag and language code: 'foo', 'foo'",
            str(cm.exception),
        )
