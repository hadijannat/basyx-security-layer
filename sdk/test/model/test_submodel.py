...
class EntityTest(unittest.TestCase):
    def test_aasd_014_init_self_managed(self) -> None:
        ...
        self.assertEqual(
            "A self-managed entity has to have a globalAssetId or a specificAssetId (Constraint AASd-014)",
            str(cm.exception),
        )
        model.Entity(
            "TestEntity",
    ...
    def test_aasd_014_init_co_managed(self) -> None:
        ...
        with self.assertRaises(model.AASConstraintViolation) as cm:
            model.Entity(
                "TestEntity",
                model.EntityType.CO_MANAGED_ENTITY,
                global_asset_id="https://acplt.org/TestAsset",
            )
        self.assertEqual(
    ...
    def test_aasd_014_set_co_managed(self) -> None:
        ...
        with self.assertRaises(model.AASConstraintViolation) as cm:
            entity.specific_asset_id = model.ConstrainedList(
                (model.SpecificAssetId("test", "test"),)
            )
        self.assertEqual(
            "A co-managed entity has to have neither a globalAssetId nor a specificAssetId "
            "(Constraint AASd-014)",
    ...
    def test_aasd_014_specific_asset_id_del_self_managed(self) -> None:
        ...
        entity = model.Entity(
            "TestEntity",
            model.EntityType.SELF_MANAGED_ENTITY,
            specific_asset_id=(model.SpecificAssetId("test", "test"), specific_asset_id),
        )
        with self.assertRaises(model.AASConstraintViolation) as cm:
...
class SubmodelElementListTest(unittest.TestCase):
    def test_constraints(self):
        ...
        self.assertEqual(
            "If semantic_id_list_element=ExternalReference(key=(Key(type=GLOBAL_REFERENCE, "
        ...
        sel.value.clear()
        model.SubmodelElementList(
            "test_list", model.MultiLanguageProperty, {mlp}, semantic_id_list_element=None
        )
        ...
        are = model.AnnotatedRelationshipElement(
            ...
            model.ExternalReference(
                (model.Key(model.KeyTypes.GLOBAL_REFERENCE, "urn:x-test:test-first"),)
            ),
            model.ExternalReference(
                (model.Key(model.KeyTypes.GLOBAL_REFERENCE, "urn:x-test:test-second"),)
            ),
        )
        ...
        self.assertEqual(
            "type_value_list_element=Property, but value_type_list_element is not set! "
            "(Constraint AASd-109)",
            str(cm.exception),
        )
        model.SubmodelElementList(
            "test_list", model.Property, [prop], value_type_list_element=model.datatypes.Int
        )
        ...
        self.assertEqual(
            "All first level elements must have the value_type specified by value_type_list_element=Int, "
            "got Property with value_type=str (Constraint AASd-109)",
            str(cm.exception),
        )
        model.SubmodelElementList(
            "test_list", model.Property, {prop}, value_type_list_element=model.datatypes.String
        )

        # AASd-114
        semantic_id1 = model.ExternalReference(
            (model.Key(model.KeyTypes.GLOBAL_REFERENCE, "urn:x-test:test"),)
        )
        semantic_id2 = model.ExternalReference(
            (model.Key(model.KeyTypes.GLOBAL_REFERENCE, "urn:x-test:different"),)
        )
        ...
        self.assertEqual(
            "Element to be added MultiLanguageProperty has semantic_id "
            ...
            "which aren't equal. (Constraint AASd-114)",
            str(cm.exception),
        )
        ...
        self.assertEqual(
            "Objects with an id_short may not be added to a SubmodelElementList, got "
            "MultiLanguageProperty[mlp] with id_short=mlp (Constraint AASd-120)",
            str(cm.exception),
        )
        mlp.id_short = None
        model.SubmodelElementList("test_list", model.MultiLanguageProperty, [mlp])
        with self.assertRaises(model.AASConstraintViolation) as cm:
            mlp.id_short = "mlp"
        self.assertEqual(
            "id_short of MultiLanguageProperty[test_list[0]] cannot be set, because it is "
            "contained in a SubmodelElementList[test_list] (Constraint AASd-120)",
            str(cm.exception),
        )
    ...
    def test_aasd_108_add_set(self):
        ...
        self.assertEqual(
            "All first level elements must be of the type specified in type_value_list_element="
            "MultiLanguageProperty, got Property (Constraint AASd-108)",
            str(cm.exception),
        )
    ...
    def test_immutable_attributes(self):
        ...
        with self.assertRaises(AttributeError):
            list_.semantic_id_list_element = model.ExternalReference(
                (model.Key(model.KeyTypes.GLOBAL_REFERENCE, "t"),)
            )
        with self.assertRaises(AttributeError):
    ...
    def test_constraints(self):
        with self.assertRaises(ValueError) as cm:
            model.BasicEventElement(
                "test_basic_event_element",
                model.ModelReference(
                    (
                        model.Key(
                            model.KeyTypes.ASSET_ADMINISTRATION_SHELL,
        ...
        timestamp_tzinfo = model.datatypes.DateTime(
            2022, 11, 13, 23, 45, 30, 123456, dateutil.tz.gettz("Europe/Berlin")
        )
        with self.assertRaises(ValueError) as cm:
        ...
        timestamp_tzinfo_utc = model.datatypes.DateTime(
            2022, 11, 13, 23, 45, 30, 123456, dateutil.tz.UTC
        )
