...
def create_test_aas():
    ...
    submodel = model.Submodel(
        id_="https://example.com/sensors/temp", id_short="TempSensor", submodel_element={temp}
    )
...
def test_security_manager():
    ...
    # Create contexts
    admin_context = create_security_context("admin1", {"admin"}, SecurityLevel.HIGH)
    user_context = create_security_context("user1", {"user"}, SecurityLevel.LOW)
...
def test_secure_aas():
    ...
    # Create contexts
    admin_context = create_security_context("admin1", {"admin"}, SecurityLevel.HIGH)
    operator_context = create_security_context("operator1", {"operator"}, SecurityLevel.MEDIUM)
...
def test_rate_limiter():
    ...
    limiter = RateLimiter()
    limit = RateLimit(
        requests=2, window_seconds=1, block_seconds=2
    )  # Use shorter block time for testing
